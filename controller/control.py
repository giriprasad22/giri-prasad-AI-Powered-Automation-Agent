...
def main(model, terminal_prompt, voice_mode=False, verbose_mode=False):
    """
    Main function for the giri-prasad.
    ...
    if terminal_prompt:  # Skip objective prompt if it was given as an argument
        ...
        print(
            f"{ANSI_GREEN}[giri-prasad]{ANSI_RESET} Listening for your command... (speak now)"
        )
        try:
        ...
        print(
            f"[{ANSI_GREEN}giri-prasad {ANSI_RESET}|{ANSI_BRIGHT_MAGENTA} {model}{ANSI_RESET}]\n{USER_QUESTION}"
        )
    ...
    while True:
        try:
            ...
            print(
                f"{ANSI_GREEN}[giri-prasad]{ANSI_RED}[Error] -> {e} {ANSI_RESET}"
            )
            break
        except Exception as e:
            print(
...
def operate(operations, model):
    for operation in operations:
        ...
        if operate_type == "press" or operate_type == "hotkey":
            ...
            # Support both label-based clicks (YOLO) and coordinate-based clicks
            label = operation.get("label")
            
            if label:
                # Label-based click from YOLO object detection
                operate_detail = f"label {label}"
                # TODO: Implement label coordinate lookup
                # For now, this will fail gracefully
                print(
                    f"{ANSI_GREEN}[giri-prasad]{ANSI_YELLOW}[Warning] Label-based clicking not yet implemented{ANSI_RESET}"
                )
            else:
            ...
            print(
                f"{ANSI_GREEN}[giri-prasad]{ANSI_RED}[Error] unknown operation response :({ANSI_RESET}"
            )
            print(
                f"{ANSI_GREEN}[giri-prasad]{ANSI_RED}[Error] AI response {ANSI_RESET}{operation}"
            )
            return True
