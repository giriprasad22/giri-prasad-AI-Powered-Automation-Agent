...
IMPORTANT RULES:
- NEVER use coordinates or click operations
- ALWAYS use the OS search to find and open applications
- Use keyboard shortcuts and typing only
- Press {os_search_str} to open search, then write the application name, then press enter
...
```
[
    {{ "thought": "I need to open Notepad. I'll use the OS search.", "operation": "press", "keys": {os_search_str} }},
    {{ "thought": "Now I'll type 'notepad' to search for it", "operation": "write", "content": "notepad" }},
    {{ "thought": "Press enter to open Notepad", "operation": "press", "keys": ["enter"] }}
]
```

Example 2: Open Google Chrome and go to a website
```
[
    {{ "thought": "I need to open Chrome using OS search", "operation": "press", "keys": {os_search_str} }},
    {{ "thought": "Type Chrome to search for it", "operation": "write", "content": "chrome" }},
    {{ "thought": "Press enter to open Chrome", "operation": "press", "keys": ["enter"] }}
]
```

Example 3: Type text in an active window
```
[
    {{ "thought": "The window is already focused, I can just type", "operation": "write", "content": "Hello World" }}
]
```

Example 4: Use keyboard shortcut to focus address bar in browser
```
[
    {{ "thought": "I'll focus on the address bar using keyboard shortcut", "operation": "press", "keys": [{cmd_string}, "l"] }},
    {{ "thought": "Now I'll type the URL", "operation": "write", "content": "https://google.com" }},
    {{ "thought": "Press enter to navigate", "operation": "press", "keys": ["enter"] }}
]
```

Key points:
- Always use {os_search_str} to open the OS search menu
- Type the application name to search
- Press enter to open it
- Use keyboard shortcuts for navigation (Ctrl+L for address bar, Ctrl+T for new tab, etc.)
- Never try to click with mouse or use coordinates
...
Please take the next best action. The `pyautogui` library will be used to execute your decision. Your output will be used in a `json.loads` loads statement. Remember you only have the following 3 operations available: write, press, done

IMPORTANT: Never use click or coordinates. Always use keyboard shortcuts and search to navigate.

You just started so you are in the terminal app and your code is running in this terminal tab. To leave the terminal and open any application, use the OS search function.

Action:"""

OPERATE_PROMPT = """
Please take the next best action. The `pyautogui` library will be used in a `json.loads` loads statement. Remember you only have the following 3 operations available: write, press, done

IMPORTANT: Never use click or coordinates. Use keyboard only.
...
def get_system_prompt(model, objective):
    ...
    # Use LABELED prompt for gemma models (with YOLO object detection)
    if model.startswith("gemma") or model == "gpt-4-with-som":
        ...
        if config.verbose:
            print(f"[get_system_prompt] Using LABELED prompt for {model}")
